...
qshome = 'C:/PortraitMiner7.0/server/qs7.0A/win64/bin'
...
def get_previous_focus(tasklist, path, name, currentstep):
    ...
    if currentstep == tasklist[0]: # If first task...
        ...
        for step in reversed(tasklist):
            if prior:
                print('looking for',name,step,path + "//" + name + "_*" + "_" + step + ".ftr")
                file = glob.glob(path + "//" + name + "_*" + "_" + step + ".ftr") or  glob.glob(path + "//" + name + "_" + step + ".ftr")
                if file:
                    file = file[0]
...
def joinprocOld(self, inp, out, rhs):
    if not os.path.isfile(rundirsub + '//' + out + '.ftr'):
        ...
        qsjoinplus(rundirsub + '//' + inp, keys, rundirsub + '//' + out + '.ftr', rundirsub + '//' + rhsfocus,
                   fields=rhsfields)
    ...
    if not os.path.isfile(rundirsub + '//' + out + '.ftr'):
        ...
        for j in rhs.split(';'):
            ...
            if j == rhs.split(';')[0]:
                ...
                qsjoinplus(rundirsub + '//' + out, keys, rundirsub + '//' + out + '.ftr', rundirsub + '//' + rhsfocus,
                           fields=rhsfields)
...
def runqsdb(command, arglist, failonbad=True):
    ...
    for a in arglist:
        args = args + ' ' + a
    print('EXECUTING in os.system:', command2, args)
    #os.system(command2)
    result = os.spawnv(os.P_WAIT, command2, [command+args])
    return result
